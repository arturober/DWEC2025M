...
export class LoadGoogleApi {
  ...
  constructor() {
    if (!this.#clientId) {
      ...
      throw new Error(
        'LoadGoogleApiService: You must call provideGoogleId with your Google ID in your providers array',
      );
    }
  ...
  get credential$() {
    return this.#credential$.asObservable(); // Desde fuera solo recibimos valores, no podemos emitir
  }
